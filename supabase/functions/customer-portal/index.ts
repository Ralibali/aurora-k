...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!parsed.success) return new Response(JSON.stringify({ error: 'Invalid token' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });

    const supabase = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);
    const tokenData = await validateToken(supabase, parsed.data.token);
    if (!tokenData) return new Response(JSON.stringify({ error: 'Invalid or expired token' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    ...
    if (req.method === 'POST') {
      ...
      if (!bookingParsed.success) return new Response(JSON.stringify({ error: 'Ogiltiga uppgifter', details: bookingParsed.error.flatten().fieldErrors }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      ...
      if (error) return new Response(JSON.stringify({ error: 'Kunde inte skapa förfrågan' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    ...
    const [assignmentsRes, ordersRes, invoicesRes, bookingsRes, settingsRes] = await Promise.all([
      ...
      supabase.from('settings').select('company_name, org_number, address, zip_city, email, phone, bankgiro, plusgiro, vat_number').eq('company_id', companyId).maybeSingle(),
    ]);
