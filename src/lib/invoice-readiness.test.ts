...
describe('invoice readiness',()=>{
 it('calculates known hourly prices and keeps zero-price deliveries valid',()=>{expect(invoiceReadiness(base)).toMatchObject({hours:2.5,amount:1250,ready:true});expect(invoiceReadiness({...base,customer:{pricing_type:'per_delivery',price_per_delivery:0,price_per_hour:null}}).ready).toBe(true);});
