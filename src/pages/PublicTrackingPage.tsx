...
function formatDate(value?: string | null) {
  if (!value) return '—';
  return new Date(value).toLocaleString('sv-SE', { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' });
}
...
export default function PublicTrackingPage() {
  ...
  const load = useCallback(async (manual = false) => {
    ...
    try {
      const response = await fetch(`https://${projectId}.supabase.co/functions/v1/track-assignment?token=${encodeURIComponent(token)}`, { cache: 'no-store' });
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-xl space-y-4 p-5">
        ...
        {active && <Card className="border-blue-200 bg-blue-50"><CardContent className="space-y-3 p-5"><div className="flex items-center justify-between"><div><p className="font-semibold text-blue-950">{driver?.firstName || 'Chauffören'} är på väg</p><p className="text-sm text-blue-800">Senast uppdaterad {location ? new Date(location.updatedAt).toLocaleTimeString('sv-SE', { hour: '2-digit', minute: '2-digit' }) : 'position saknas'}</p></div><RefreshCw className={`h-5 w-5 text-blue-700 ${refreshing ? 'animate-spin' : ''}`} /></div>{location && <Button asChild className="w-full"><a href={mapsUrl} target="_blank" rel="noreferrer"><MapPin className="mr-2 h-4 w-4" />Visa senaste position</a></Button>}{driver?.phone && <Button asChild variant="outline" className="w-full bg-white"><a href={`tel:${driver.phone}`}><Phone className="mr-2 h-4 w-4" />Ring {driver.firstName}</a></Button>}</CardContent></Card>}
