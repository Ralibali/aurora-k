...
export default function DriverAssignmentDetail() {
  ...
  if (!assignment) {
    return <div className="p-8 text-center text-muted-foreground">Uppdraget hittades inte</div>;
  }
  ...
  const saveDriverComment = (nextComment: string, successText: string) => {
    updateAssignment.mutate(
      { id: assignment.id, driver_comment: nextComment || null },
  ...
  const handleStart = () => {
    updateAssignment.mutate({
      ...
      driver_comment: appendEvent(assignment.driver_comment as string | null, 'Körning startad'),
    });
  };
  ...
  const handleSaveComment = () => {
    saveDriverComment(driverComment, 'Kommentar sparad');
  };
    ...
    <div className="min-h-screen bg-background pb-28">
      <div className="sticky top-0 z-20 border-b bg-background/95 px-5 py-4 backdrop-blur">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(-1)} className="flex h-10 w-10 items-center justify-center rounded-full bg-muted active:scale-95">
      ...
      <main className="space-y-5 p-5">
        <Card className="overflow-hidden">
          <CardHeader className="bg-slate-950 text-white">
            <CardTitle className="flex items-center gap-2 text-xl">
              <Truck className="h-5 w-5" /> Ditt uppdrag
            </CardTitle>
          ...
          <CardContent className="px-4">
            ...
            <InfoRow icon={MapPin} label="Adress">
              <button onClick={() => openMaps(assignment.address)} className="flex items-center gap-1 text-left text-primary">
                {assignment.address}<Navigation className="h-3.5 w-3.5" />
              </button>
            </InfoRow>
            ...
            {assignment.actual_start && assignment.actual_stop && (
              <InfoRow icon={CheckCircle2} label="Varaktighet">{calculateDuration(assignment.actual_start, assignment.actual_stop)}</InfoRow>
            )}
          </CardContent>
        </Card>
        ...
        {assignment.admin_comment && (
          <Card className="border-blue-200 bg-blue-50">
            <CardContent className="p-4">
              <p className="text-xs font-semibold uppercase text-blue-700">Meddelande från admin</p>
              <p className="mt-1 whitespace-pre-wrap text-sm text-slate-900">{assignment.admin_comment}</p>
            </CardContent>
          </Card>
        )}
        ...
        {!isCompleted && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Snabbstatus till admin</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-2 sm:grid-cols-2">
              ...
              <Button variant="outline" onClick={() => setShowDeviation(v => !v)} className="border-amber-300 text-amber-700">
                <AlertTriangle className="mr-2 h-4 w-4" /> Rapportera avvikelse
              </Button>
            </CardContent>
          </Card>
        )}
        ...
        {showDeviation && (
          <Card className="border-amber-200 bg-amber-50">
            <CardContent className="space-y-3 p-4">
              <p className="font-semibold text-amber-800">Vad har hänt?</p>
              <Textarea value={deviationText} onChange={e => setDeviationText(e.target.value)} placeholder="T.ex. kund ej på plats, fel adress, gods saknas, skada upptäckt..." />
              <Button onClick={handleDeviation} disabled={updateAssignment.isPending} className="w-full">Skicka avvikelse</Button>
            </CardContent>
          </Card>
        )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base"><MessageSquare className="h-4 w-4" /> Förarkommentar</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <Textarea rows={6} value={driverComment} onChange={e => setDriverComment(e.target.value)} placeholder="Skriv anteckning, status eller information till admin..." />
            <Button variant="outline" onClick={handleSaveComment} disabled={updateAssignment.isPending} className="w-full">
              <Send className="mr-2 h-4 w-4" /> Spara kommentar
            </Button>
          </CardContent>
        </Card>
        ...
        {isCompleted && (
          <Card className="border-green-200 bg-green-50">
            <CardContent className="space-y-3 p-5 text-center">
              ...
              <p className="text-lg font-bold text-green-900">Uppdraget är slutfört</p>
              {assignment.actual_start && assignment.actual_stop && (
                <p className="text-sm text-green-800">Tid: {calculateDuration(assignment.actual_start, assignment.actual_stop)}</p>
              )}
              {a.consignment_photo_url && <img src={a.consignment_photo_url} alt="Fraktsedel" className="mx-auto mt-3 max-w-xs rounded-xl border bg-white" />}
            </CardContent>
          </Card>
        )}
      </main>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t bg-card/95 px-5 py-3 backdrop-blur" style={{ paddingBottom: 'max(12px, env(safe-area-inset-bottom))' }}>
        {isPending && (
          <Button onClick={handleStart} disabled={updateAssignment.isPending} className="h-14 w-full text-base">
            <Play className="mr-2 h-5 w-5" /> Starta körning
          </Button>
        )}
        {isActive && (
          <div className="grid grid-cols-2 gap-2">
            <Button variant="outline" onClick={handleTakePhotoAndComplete} disabled={updateAssignment.isPending} className="h-14">
              <Camera className="mr-2 h-5 w-5" /> Foto & klar
            </Button>
            <Button onClick={() => completeAssignment(null)} disabled={updateAssignment.isPending} className="h-14 bg-green-600 hover:bg-green-700">
              <CheckCircle2 className="mr-2 h-5 w-5" /> Slutför
            </Button>
          </div>
        )}
        {isCompleted && (
          <Button variant="outline" onClick={() => navigate('/driver/assignments')} className="h-14 w-full">Till mina uppdrag</Button>
        )}
      </div>
    </div>
  );
