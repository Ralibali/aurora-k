    ...
    <AdminLayout title="Uppdrag" description="Transportledning, rutter och förare">
      <div className="space-y-5">
        ...
        {isLoading ? (
          ...
          effectiveAssignments.length === 0 ? <EmptyState icon={InboxIcon} title="Skapa ditt första uppdrag" description="Samla körningar, förare, tider och kundinformation på ett ställe." hint="Tips: aktivera Demo mode för att visa en säljklar version med exempeldata." actionLabel="Skapa uppdrag" actionHref="/admin/assignments/new" secondaryLabel="Aktivera Demo mode" onSecondary={demo.enable} /> : <div className="bg-card rounded-lg border border-dashed border-border p-16 text-center shadow-card"><Inbox className="h-12 w-12 text-muted-foreground/30 mx-auto mb-3" /><p className="text-sm font-medium text-muted-foreground">Inga uppdrag matchar filtren</p><Button size="sm" variant="outline" className="mt-4" onClick={() => { setStatusFilter('all'); setDriverFilter('all'); setSearch(''); }}>Rensa filter</Button></div>
        ) : (
          <>
