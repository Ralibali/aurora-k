...
export default function AdminRouteOptimizerPage() {
  ...
  const availableAssignments = useMemo(() => {
    ...
    return (assignments ?? [])
      .filter(item => item.assigned_driver_id === selectedDriver && getStockholmDateKey(item.scheduled_start) === selectedDate && isOpenAssignment(item))
    ...
    <AdminLayout title="Slingor och ruttplanering" description="Ordna och spara dagens stopp till föraren">
      <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <>{mapsAvailable && <Button variant="outline" onClick={calculateRoad} disabled={routing || !orderedAssignments.length}>{routing ? 'Beräknar…' : 'Beräkna körväg'}</Button>}</>
