...
export default function AdsFlottaPage() {
  ...
  useBreadcrumbJsonLd(useMemo(() => [
    ...
    { name: 'Fordonshantering', url: 'https://auroratransport.se/ads/flotta' },
  ], []));
  ...
  return (
    <div className="min-h-screen bg-[#050510] text-white overflow-hidden" style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] rounded-full bg-amber-600/10 blur-[120px]" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] rounded-full bg-orange-500/8 blur-[100px]" />
        <div className="absolute top-[40%] right-[20%] w-[300px] h-[300px] rounded-full bg-yellow-400/5 blur-[80px]" />
      </div>
      ...
      <div className="relative z-10">
            ...
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              <motion.div>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                  <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-amber-500/20 bg-amber-500/5 text-sm font-medium text-amber-400 mb-6 backdrop-blur-sm">
                    <span className="h-1.5 w-1.5 rounded-full bg-amber-400 animate-pulse" />
                    Fordonshantering & flotta
                  </span>
                </motion.div>
                ...
                <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }} className="text-lg sm:text-xl text-white/50 leading-relaxed mb-8 max-w-lg">
                  Se var alla fordon är, vem som kör, och när nästa service behövs. Allt i realtid.
                </motion.p>
                ...
                <motion.div {...glowPulse} className="relative">
                  <div className="bg-gradient-to-br from-[#0d1225] to-[#0a0f1e] rounded-2xl border border-white/[0.06] p-6 shadow-2xl">
                    <div className="flex items-center justify-between mb-5">
                      <span className="text-xs text-white/40 font-mono">FLOTTAÖVERSIKT</span>
                      <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" /><span className="text-xs text-white/30 font-mono">LIVE</span></div>
                    </div>
                    <div className="grid grid-cols-4 gap-2 mb-5">
                      {[{ label: 'Totalt', value: '15', color: 'text-white/80' }, { label: 'I trafik', value: '9', color: 'text-green-400' }, { label: 'Depå', value: '4', color: 'text-amber-400' }, { label: 'Service', value: '2', color: 'text-rose-400' }].map(s => (
                        <div key={s.label} className="bg-white/[0.03] rounded-xl p-2.5 border border-white/[0.04] text-center">
                    ...
                    {[
                      { reg: 'ABC 123', type: 'Lastbil', km: '234 512 km', status: 'I trafik', color: 'bg-green-400' },
                      { reg: 'DEF 456', type: 'Skåpbil', km: '89 230 km', status: 'Service snart', color: 'bg-amber-400' },
                      { reg: 'GHI 789', type: 'Dragbil', km: '412 001 km', status: 'På depå', color: 'bg-white/30' },
                    ].map((v, i) => (
                      <motion.div key={v.reg} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 + i * 0.12 }} className="flex items-center gap-3 py-2.5 border-b border-white/[0.04] last:border-0">
                        <div className="w-8 h-8 rounded-lg bg-white/[0.04] flex items-center justify-center"><Car className="h-4 w-4 text-amber-400/70" /></div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 text-sm"><span className="text-white/70 font-medium font-mono">{v.reg}</span><span className="text-white/25">·</span><span className="text-white/40">{v.type}</span></div>
                          <span className="text-xs text-white/25">{v.km}</span>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0"><span className={`h-1.5 w-1.5 rounded-full ${v.color}`} /><span className="text-xs text-white/40">{v.status}</span></div>
                      </motion.div>
                    ))}
                  </div>
                </motion.div>
        ...
        <div className="border-y border-white/[0.04] py-5 bg-white/[0.01]">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-12">
              {['Ingen bindningstid', 'Obegränsat antal fordon', 'GPS-spårning ingår', 'Support på svenska'].map(item => (
                <span key={item} className="flex items-center gap-2 text-sm text-white/30"><span className="h-1 w-1 rounded-full bg-amber-400" />{item}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <section className="py-24">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            <motion.p initial="hidden" whileInView="visible" viewport={{ once: true }} custom={1} variants={fadeUp} className="text-center text-white/30 mb-16 max-w-md mx-auto">De flesta med fordonsflottor saknar överblick.</motion.p>
            <div className="grid md:grid-cols-3 gap-6">
              {[
                { emoji: '🗺️', title: 'Var är fordonen?', desc: 'Ingen vet var bilarna befinner sig. Ring-och-fråga varje gång.' },
                { emoji: '🔧', title: 'Missade serviceintervall', desc: 'Besiktningar missas. Däck byts för sent. Kostnaderna ökar.' },
                { emoji: '📱', title: 'Ingen digital översikt', desc: 'Allt finns i någons huvud eller i ett Excel-ark som sällan uppdateras.' },
              ].map((p, i) => (
        ...
        <section className="py-24">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} variants={fadeUp} className="text-center mb-4">
              <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-amber-500/20 bg-amber-500/5 text-sm font-medium text-amber-400 mb-4">Komplett flottahantering</span>
            </motion.div>
            <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} variants={fadeUp} className="text-3xl sm:text-4xl font-bold text-center mb-16">Allt du behöver för din flotta.</motion.h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {[
                { icon: Car, title: 'Fordonsregister', desc: 'Registrera alla fordon med typ, reg.nr, modell och årtal.', color: 'from-amber-500/20 to-amber-600/5 border-amber-500/10' },
                { icon: MapPin, title: 'GPS-spårning', desc: 'Se alla fordon på live-kartan. Position, hastighet och riktning.', color: 'from-cyan-500/20 to-cyan-600/5 border-cyan-500/10' },
                { icon: Users, title: 'Förarkoppling', desc: 'Koppla förare till fordon. Se vem som kör vad just nu.', color: 'from-violet-500/20 to-violet-600/5 border-violet-500/10' },
                { icon: Gauge, title: 'Körsträcka & förbrukning', desc: 'Spåra kilometer, bränsleförbrukning och CO2-utsläpp.', color: 'from-green-500/20 to-green-600/5 border-green-500/10' },
                { icon: Clock, title: 'Tidrapportering', desc: 'Automatisk in/ut-stämpling per uppdrag och förare.', color: 'from-teal-500/20 to-teal-600/5 border-teal-500/10' },
                { icon: BarChart3, title: 'Kostnadsanalys', desc: 'Se kostnader per fordon, förare och kund i tydliga rapporter.', color: 'from-rose-500/20 to-rose-600/5 border-rose-500/10' },
              ].map((f, i) => (
        ...
        <section className="py-24">
          <div className="max-w-3xl mx-auto px-4 sm:px-6">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative">
              <div className="absolute inset-0 bg-gradient-to-b from-amber-600/10 to-transparent rounded-3xl blur-xl" />
              <div className="relative bg-gradient-to-br from-white/[0.04] to-white/[0.01] border border-white/[0.06] rounded-3xl p-8 sm:p-10">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {['Obegränsat antal fordon', 'GPS & live-karta', 'Förarapp (PWA)', 'Fordonsregister & historik', 'Körsträcka & CO2-spårning', 'Uppdragshantering', 'Tidrapportering & OB', 'Support på svenska'].map(item => (
                    <div key={item} className="flex items-center gap-2.5 py-2"><div className="w-5 h-5 rounded-full bg-amber-500/10 flex items-center justify-center shrink-0"><Check className="h-3 w-3 text-amber-400" /></div><span className="text-sm text-white/50">{item}</span></div>
                  ))}
                </div>
                <div className="mt-8 pt-6 border-t border-white/[0.04] flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <button onClick={handleDemo} disabled={demoLoading} className="text-sm text-amber-400 hover:text-amber-300 transition-colors flex items-center gap-1.5"><Play className="h-3.5 w-3.5" /> Testa demo först</button>
                </div>
              </div>
            </motion.div>
          </div>
        ...
        <section className="py-24 relative">
          <div className="absolute inset-0 pointer-events-none"><div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-amber-600/5 blur-[120px] rounded-full" /></div>
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-[#050510]/90 backdrop-blur-xl border-b border-white/5' : ''}`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2.5 group">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-amber-500 to-amber-700 flex items-center justify-center shadow-lg shadow-amber-500/20 group-hover:shadow-amber-500/40 transition-shadow"><Truck className="h-4 w-4 text-white" /></div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button size="sm" asChild className="bg-amber-600 hover:bg-amber-500 text-white rounded-lg shadow-lg shadow-amber-600/20 hover:shadow-amber-500/30 transition-all"><Link to="/kontakt">Kom igång</Link></Button>
        </div>
      </div>
    </nav>
  );
