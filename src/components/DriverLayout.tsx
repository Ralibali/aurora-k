...
import { ClipboardList, User, LogOut, Clock, Truck, Home, Menu, Plus } from 'lucide-react';
...
export function DriverLayout({ children }: DriverLayoutProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <Link
        to="/driver"
        className="fixed right-4 bottom-20 z-50 w-12 h-12 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center hover:bg-primary/90 transition-colors pb-safe"
      >
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-card border-t flex z-50 pb-safe">
        <NavLink
          ...
          className="flex-1 flex flex-col items-center justify-center py-2.5 text-muted-foreground transition-colors duration-100 touch-target"
