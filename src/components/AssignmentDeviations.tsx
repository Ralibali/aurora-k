...
export default function AssignmentDeviations({assignmentId,canResolve=false,legacyComment,driverReporting=false}:{assignmentId:string;canResolve?:boolean;legacyComment?:string|null;driverReporting?:boolean}) {
 ...
 return <Card className="border-amber-200"><CardHeader><CardTitle className="text-base flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-amber-600"/>{driverReporting?'Rapportera problem':'Avvikelser och åtgärder'}</CardTitle></CardHeader><CardContent className="space-y-4">
 <p className="text-sm text-muted-foreground">{driverReporting?'Rapportera problem med uppdraget, olämpligt innehåll eller en persons beteende. Rapporten blir synlig för företagets administratör som kan dokumentera en åtgärd.':'Beskriv vad som hänt. Administratören dokumenterar åtgärden och avslutar avvikelsen före fakturering.'}</p>
 ...
 {driverReporting?<ProblemReportForm includeAssignmentProblem successMessage="Rapporten är sparad och synlig för företagets administratör." onSubmit={async(message,id)=>{await change.mutateAsync({type:'report',assignmentId,message,operationId:id});}}/>:<><Button variant="outline" disabled={change.isPending} onClick={()=>setOpen(value=>!value)}>{open?'Stäng formuläret':'Rapportera avvikelse'}</Button>
