...
export function DeliveryProofDialog({ open, onOpenChange, assignment, userId, companyId, onComplete }: {
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success('Leveransbevis sparat');
      onOpenChange(false);
    } catch (error: unknown) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] max-w-lg overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <Button className="h-12 w-full bg-green-600 hover:bg-green-700" disabled={saving} onClick={() => void submit()}>{saving ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Sparar…</> : <><CheckCircle2 className="mr-2 h-4 w-4" />Slutför uppdrag</>}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
